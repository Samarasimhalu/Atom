...
test('approval workflow binds an approver decision to one requester, specification, session, and idempotency key', async () => {
  ...
  const request = { tenantId: 'tenant', requesterId: 'requester', specification: spec, sessionId: 'session-12345678', idempotencyKey: 'idem-1', policy: { approvalRequired: true }, policyVersion: 'policy-v1', intendedAction: 'execute_test' };
  ...
  await assert.rejects(() => workflow.consume(approval.id, { ...request, tenantId: 'other-tenant' }), /approval_not_found_or_not_authorized/);
  await assert.rejects(() => workflow.consume(approval.id, { ...request, specification: { ...spec, purpose: 'altered execution' } }), /approval_request_mismatch/);
  await assert.rejects(() => workflow.consume(approval.id, { ...request, specification: { ...spec, target: { ...spec.target, url: 'https://shop.example/other' } } }), /approval_request_mismatch/);
  await assert.rejects(() => workflow.consume(approval.id, { ...request, specification: { ...spec, tags: [...spec.tags, 'destructive'] } }), /approval_request_mismatch/);
  await assert.rejects(() => workflow.consume(approval.id, { ...request, policyVersion: 'policy-v2' }), /approval_request_mismatch/);
  await assert.rejects(() => workflow.consume(approval.id, { ...request, intendedAction: 'cancel_run' }), /approval_request_mismatch/);
  ...
  const audit = await store.listAudit('tenant');
  assert.equal(audit.filter(event => event.action === 'approval.consume_denied').length, 7);
  assert.ok(audit.some(event => event.action === 'approval.consumed'));
  await store.close();
